...
| `import_export_preserves_provenance_and_lifecycle` | `export_profile`, `import_profile`, `export_bundle`, `verify_bundle`, `import_bundle` | `golden_trace_portable_bundle_manifest_roundtrip`, `golden_trace_import_restores_lifecycle_tombstones`, `golden_trace_import_preserves_policy_metadata`, `golden_trace_import_preserves_graph_evidence_chains` |
| `auditable_memory_actions` | `remember`, `before_model_call`, `memory_changes`, `explain_router_run`, `export_profile` | `professional_memory_injected_with_provenance`, `keeper_change_is_inspectable`, `golden_trace_graph_browser_shows_source_previews` |
...
- corrupted-store and interrupted import/export recovery fixtures;
