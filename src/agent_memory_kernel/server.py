...
def handle_api_request(store: MemoryStore, path: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if path == "/health":
        return {"status": "ok"}
    if path in {"/kernel/status", "/kernel-status"}:
        return store.kernel_status(
    ...
    if path in {"/memory/explain", "/memory-explain"}:
        return store.explain_memory(str(payload.get("memory_id", "")))
